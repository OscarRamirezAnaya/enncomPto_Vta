...
try
{
    ...
    {
        if($f['columna'] == 'name') $wh .= " AND CONCAT(name, ' ', app) LIKE '%" . addslashes ($f['valor']) . "%'";
        if($f['columna'] == 'email') $wh .= " AND email LIKE '%" . addslashes ($f['valor']) . "%'";
        if($f['columna'] == 'tel') $wh .= " AND tel LIKE '%" . addslashes ($f['valor']) . "%'";
    ...
    //$db = new PDO("mysql:dbname=db598436933;host=db598436933.db.1and1.com;charset=utf8", "dbo598436933", "oscar01" );
    ...
    // {
    //     $profesion = $db->query("SELECT * FROM clientes p WHERE p.id = " . $r['Profesion_id'])
    //                     ->fetch(PDO::FETCH_ASSOC);
        
    //     $registros[$k]['clientes'] = $profesion;
    // }
